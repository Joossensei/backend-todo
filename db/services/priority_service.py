...
class PriorityService:
    ...
    async def create_priority(
        ...
        db_priority = await conn.fetchrow(
            'INSERT INTO priorities ("key", "name", "description", "color", "icon", "order", "user_key") VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING *',
            priority["key"],
            priority["name"],
    ...
    async def fetch_priority_id_by_key(
        ...
        db_priority = await conn.fetchrow(
            "SELECT id FROM priorities WHERE key = $1 AND user_key = $2", key, user_key
        )
    ...
    async def get_priorities(
        ...
        return await conn.fetch(
            'SELECT * FROM priorities WHERE user_key = $1  ORDER BY "order" ASC OFFSET $2 LIMIT $3',
            user_key,
            skip,
            limit,
        )
    ...
    async def get_priority(conn: asyncpg.Connection, priority_id: int, user_key: str):
        return await conn.fetchrow(
            "SELECT * FROM priorities WHERE id = $1 AND user_key = $2",
            priority_id,
            user_key,
        )
    ...
    async def delete_priority(
        ...
        await conn.execute(
            "DELETE FROM priorities WHERE id = $1 AND user_key = $2",
            priority_id,
            user_key,
        )
    ...
    async def get_total_priorities(conn: asyncpg.Connection, user_key: str) -> int:
        return await conn.fetchval(
            "SELECT COUNT(*) FROM priorities WHERE user_key = $1",
            user_key,
        )
    ...
    async def patch_priority(
        ...
        await conn.execute(
            "UPDATE priorities SET name = $1, description = $2, color = $3, icon = $4, order = $5 WHERE id = $6 AND user_key = $7",
            db_priority.name,
    ...
    async def check_availability(
        ...
        db_priority = await conn.fetchrow(
            "SELECT * FROM priorities WHERE name = $1 AND user_key = $2",
            priority.name,
            user_key,
        )
        if db_priority:
        ...
        db_priority = await conn.fetchrow(
            "SELECT * FROM priorities WHERE order = $1 AND user_key = $2",
            priority.order,
            user_key,
        )
        if db_priority:
